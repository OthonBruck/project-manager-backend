...
async def get_visible_projects(service=Depends(get_service(ProjectService)), current_user=Depends(get_current_user)):
...
async def update_task(project_id, project_data: ProjectUpdate, service=Depends(get_service(ProjectService)), current_user=Depends(get_current_user)):
...
async def add_member(project_id, service=Depends(get_service(ProjectService)), current_user=Depends(get_current_user)):
