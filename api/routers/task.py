...
async def register_task(task: TaskCreate, db=Depends(get_database), current_user=Depends(get_current_user)):
...
async def get_task(id, db=Depends(get_database), current_user=Depends(get_current_user)):
